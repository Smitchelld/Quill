...
\usepackage[a4paper, top=1.25in, bottom=1.0in, left=1.5in, right=1.0in]{geometry}
\usepackage{graphicx}
\usepackage{epstopdf}
\usepackage{booktabs}
...
\newcommand{\aghdept}{EAIiIB / Katedra Informatyki Stosowanej}
\newcommand{\aghuni}{Akademia Górniczo-Hutnicza im. Stanisława Staszica w Krakowie}
...
\newcommand{\aghTitlePage}{%
  \thispagestyle{empty}
  \begin{center}
    \vspace*{\stretch{2}}
    \textbf{\Large\textsc{Dokumentacja techniczna projektu}}\\[0.6em]
    \textbf{\Large\textsc{Quill}}\\[0.4em]
    \textbf{\large Post-kwantowy komunikator P2P z szyfrowaniem sesji}
    \vspace*{\stretch{1}}
    \textbf{\large\textsc{Studio Projektowe, semestr 6}}\\
    \textbf{\large\textsc{Informatyka i Systemy Inteligentne}}
    \vspace*{\stretch{2}}
    {\large Autorzy projektu}\\[0.8em]
    {\large Szymon Kowalski}\\[0.3em]
    {\large Micha{\l} Kr\'{o}l}
    \vfill
    \includegraphics[height=40mm]{agh_logo.pdf}\\[1.0em]
    \large \aghdept\\[0.4em]
    {\Large \aghuni}\\[0.4em]
    Krak\'{o}w, Polska\\[1.2em]
    Czerwiec 2026 \quad wersja 1.0.0
    \vspace*{\stretch{1}}
  \end{center}
}
...
\textbf{Zakres zrealizowany} obejmuje m.in.: handshake PQC, trzy poziomy bezpieczeństwa (FAST/BALANCED/MAX), Perfect Forward Secrecy, pokoje rozmów z trwałym zapisem i hasłem, interfejs graficzny z narzędziami audytowymi, 105 testów automatycznych oraz pipeline CI.
...
\textbf{Decyzja projektowa:} separacja \texttt{quill\_core} od UI umożliwia uruchomienie 105 testów i CI bez GLFW/OpenGL/ImGui. Ta sama logika kryptograficzna działa w aplikacji i w testach integracyjnych (\texttt{socketpair}).
...
\textbf{RoomStore} (\texttt{rooms.json} w katalogu profilu) utrwala listę pokoi między uruchomieniami serwera. Pokój \texttt{general} jest domyślny i nieusuwalny. Pokoje chronione hasłem przechowują weryfikator Argon2id (salt + hash). Protokół obsługuje m.in.\ \texttt{CREATE\_ROOM}, \texttt{ROOM\_CREATED}, \texttt{ROOM\_EXISTS}, \texttt{ROOM\_DELETED}, \texttt{JOIN\_DENIED} oraz \texttt{ROOM\_LIST} z flagą \texttt{protected}. Poziom PQC ustala serwer (\texttt{SERVER\_CONFIG} przed handshake).
...
Quill/
  ...
  tests/                   # 12 plików testowych, 105 przypadków
  third_party/
...
Profil = katalog z \texttt{profile.json}, podkatalogiem \texttt{identity/}, plikami \texttt{known\_hosts}, \texttt{rooms.json} oraz opcjonalnie \texttt{.session.lock} (blokada jednej aktywnej sesji GUI).
...
\textbf{Logout:} rozłącza sesję sieciową, czyści passphrase i klucze tajne z pamięci (\texttt{OPENSSL\_cleanse}). \textbf{Usuwanie profilu} wymaga potwierdzenia hasłem w oknie modalnym.
...
\textbf{Packet Inspector} --- podgląd ostatnich pakietów w formie hex + ASCII (edukacyjny, do audytu protokołu).
...
\begin{enumerate}
    ...
    \item Oceń kompromis bezpieczeństwo vs.\ wydajność dla wybranego poziomu.
\end{enumerate}
...
\subsection{Katalog testów (105 przypadków)}
...
\subsubsection{test\_profile.cpp (11)}
...
\subsubsection{test\_roomstore.cpp (6)}
Default \texttt{general}, protected room verify, persistence across reload, duplicate add throws, remove room, cannot remove \texttt{general}.

\subsubsection{test\_truststore.cpp (8)}
First contact UNVERIFIED, second KNOWN, mark VERIFIED, algo rotation when allowed, MISMATCH preserves entry, remove allows fresh TOFU, distinct peers per host:port, corrupted known\_hosts hard error.
...
cmake -B build -G Ninja -DCMAKE_BUILD_TYPE=Release -DQUILL_BUILD_GUI=OFF
cmake --build build -j$(nproc)
./build/quill_tests              # bezposrednio
...
\begin{itemize}
    ...
    \item 105 testów z scenariuszami ataku (MITM, tamper, injection).
...
\begin{itemize}
    \item \textbf{Szymon Kowalski} --- kryptografia i protokół (\texttt{CryptoManager}, TOFU, HKDF), profile i \texttt{RoomStore}, testy automatyczne,
    \item \textbf{Micha{\l} Kr\'{o}l} --- warstwa sieciowa, interfejs ImGui (\texttt{ChatApp}/\texttt{ChatAppRender}), transfer plików, integracja UI z protokołem,
    \item \textbf{Wspólnie} --- review bezpieczeństwa, dokumentacja techniczna, scenariusze testowe.
\end{itemize}
...
\begin{itemize}
    ...
    \item trzema poziomami bezpieczeństwa (ustawianymi przez serwer),
    \item szyfrowaną sesją AES-256-GCM z anty-replay,
    \item TOFU fail-closed,
    \item profilami, pokojami z hasłem i szyfrowaniem kluczy Argon2id,
    \item transferem plików z SHA-3 i selective repeat,
    \item interfejsem ImGui z narzędziami edukacyjnymi,
    \item 105 testami i CI.
\end{itemize}

Projekt jest gotowy do oddania w ramach Studio Projektowego, z jawnie opisanymi ograniczeniami architektonicznymi (hub-and-spoke, brak NAT traversal).

\subsection{Możliwe kierunki rozwoju (poza zakresem studiów)}

\begin{itemize}
    \item NAT traversal (UDP hole punching + rendezvous),
    \item streaming plików bez pełnego buforu RAM,
    \item wersjonowanie protokołu JSON,
    \item prawdziwy end-to-end bez zaufania do serwera-relay.
\end{itemize}
...
\section{Załącznik E: Format pliku rooms.json}
...
Plik zapisywany atomowo w katalogu aktywnego profilu (\texttt{RoomStore}), chmod 0600.

\section{Załącznik F: Format pliku known\_hosts}
...
\section{Załącznik G: Lista plików źródłowych quill\_core}
